...
public interface LabAttendanceRepository extends JpaRepository<LabAttendance, Long> {
    ...
    // 根据学生ID查找实验室记录
    List<LabAttendance> findByStudentId(String studentId);

    // 根据学生ID查找并按时间排序
    List<LabAttendance> findByStudentIdOrderByCheckInTimeDesc(String studentId);

    // 分页版本
    List<LabAttendance> findByStudentIdOrderByCheckInTimeDesc(String studentId, Pageable pageable);
    ...
    // 根据学生ID和实验室名称查找记录
    List<LabAttendance> findByStudentIdAndLabName(String studentId, String labName);

    // 根据签到时间范围查找记录
    List<LabAttendance> findByCheckInTimeBetween(LocalDateTime start, LocalDateTime end);

    // 根据学生ID和签到时间范围查找记录
    List<LabAttendance> findByStudentIdAndCheckInTimeBetween(String studentId, LocalDateTime start, LocalDateTime end);

    // 获取学生的实验室总时长（分钟）
    @Query("SELECT COALESCE(SUM(a.durationMinutes), 0) FROM LabAttendance a WHERE a.studentId = :studentId")
    Integer findTotalLabMinutesByStudentId(@Param("studentId") String studentId);

    // 获取学生的实验室访问次数
    @Query("SELECT COUNT(a) FROM LabAttendance a WHERE a.studentId = :studentId")
    Long countByStudentId(@Param("studentId") String studentId);

    // 计算完成的实验数量
    @Query("SELECT COUNT(a) FROM LabAttendance a WHERE a.studentId = :studentId AND a.checkOutTime IS NOT NULL")
    Long countCompletedLabs(@Param("studentId") String studentId);
    ...
    // 获取学生最近几次实验室记录
    List<LabAttendance> findTop5ByStudentIdOrderByCheckInTimeDesc(String studentId);

    // 检查是否存在未签退的记录
    @Query("SELECT a FROM LabAttendance a WHERE a.studentId = :studentId AND a.checkOutTime IS NULL")
