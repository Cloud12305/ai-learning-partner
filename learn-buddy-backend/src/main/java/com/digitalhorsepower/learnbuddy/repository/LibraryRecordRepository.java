...
public interface LibraryRecordRepository extends JpaRepository<LibraryRecord, Long> {
    ...
    // 根据学生ID查找图书馆记录
    List<LibraryRecord> findByStudentId(String studentId);

    // 根据学生ID查找并按借阅日期排序
    List<LibraryRecord> findByStudentIdOrderByBorrowDateDesc(String studentId);

    // 分页版本
    List<LibraryRecord> findByStudentIdOrderByBorrowDateDesc(String studentId, Pageable pageable);

    // 根据学生ID和图书分类查找记录
    List<LibraryRecord> findByStudentIdAndBookCategory(String studentId, String bookCategory);
    ...
    // 查找当前借阅中的图书（未归还）
    @Query("SELECT l FROM LibraryRecord l WHERE l.studentId = :studentId AND l.returnDate IS NULL")
    List<LibraryRecord> findCurrentBorrowedBooks(@Param("studentId") String studentId);

    // 查找超期未还的图书
    @Query("SELECT l FROM LibraryRecord l WHERE l.studentId = :studentId AND l.returnDate IS NULL AND l.dueDate < :currentDate")
    List<LibraryRecord> findOverdueBooks(@Param("studentId") String studentId, @Param("currentDate") LocalDate currentDate);

    // 获取学生的借阅书籍总数
    @Query("SELECT COUNT(l) FROM LibraryRecord l WHERE l.studentId = :studentId")
    Long countByStudentId(@Param("studentId") String studentId);

    // 获取学生当前借阅数量
    @Query("SELECT COUNT(l) FROM LibraryRecord l WHERE l.studentId = :studentId AND l.returnDate IS NULL")
    Long countCurrentBorrowedByStudentId(@Param("studentId") String studentId);

    // 计算当前借阅数量
    @Query("SELECT COUNT(l) FROM LibraryRecord l WHERE l.studentId = :studentId AND l.returnDate IS NULL")
    Long countCurrentBorrowedBooks(@Param("studentId") String studentId);
    ...
    @Query("SELECT l.bookAuthor, COUNT(l) FROM LibraryRecord l " +
            "WHERE l.studentId = :studentId AND l.bookAuthor IS NOT NULL " +
    ...
    // 获取最近借阅的书籍
    List<LibraryRecord> findTop10ByStudentIdOrderByBorrowDateDesc(String studentId);

    // 根据借阅日期范围查找记录
    List<LibraryRecord> findByStudentIdAndBorrowDateBetween(String studentId, LocalDate start, LocalDate end);

    // 检查图书是否已被特定学生借阅且未归还
    @Query("SELECT COUNT(l) > 0 FROM LibraryRecord l WHERE l.studentId = :studentId AND l.bookId = :bookId AND l.returnDate IS NULL")
