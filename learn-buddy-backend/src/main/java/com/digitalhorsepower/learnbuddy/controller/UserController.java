...
public class UserController {
    ...
    // 获取所有用户（仅管理员）
    @GetMapping
    public ApiResponse<List<User>> getAllUsers(@RequestAttribute String userRole) {
        if (!"ADMIN".equals(userRole)) {
            return ApiResponse.error("权限不足，仅管理员可查看所有用户");
        }
    ...
    public ApiResponse<User> getUserById(@PathVariable Long id,
        ...
        if ("STUDENT".equals(userRole) && !userId.equals(id)) {
            return ApiResponse.error("权限不足，只能查看自己的信息");
        }
    ...
    public ApiResponse<User> createUser(@RequestBody User user,
                                        @RequestAttribute String userRole) {
        if (!"ADMIN".equals(userRole)) {
            return ApiResponse.error("权限不足，仅管理员可创建用户");
        }
    ...
    public ApiResponse<User> createAdminUser(@RequestBody User user,
                                             @RequestAttribute String userRole) {
        if (!"ADMIN".equals(userRole)) {
            return ApiResponse.error("权限不足，仅管理员可创建管理员用户");
        }
        ...
        try {
            ...
            return ApiResponse.success("管理员用户创建成功", createdUser);
        } catch (RuntimeException e) {
    ...
    public ApiResponse<User> updateUser(@PathVariable Long id,
        ...
        // 学生只能更新自己的信息
        if ("STUDENT".equals(userRole) && !userId.equals(id)) {
            return ApiResponse.error("权限不足，只能更新自己的信息");
        }
    ...
    public ApiResponse<User> updateUserRole(@PathVariable Long id,
                                            ...
                                            @RequestAttribute String userRole) {
        if (!"ADMIN".equals(userRole)) {
            return ApiResponse.error("权限不足，仅管理员可修改用户角色");
        }

        String role = request.get("role");
        if (role == null || (!"STUDENT".equals(role) && !"ADMIN".equals(role))) {
            return ApiResponse.error("角色必须是 STUDENT 或 ADMIN");
        }
    ...
    public ApiResponse<String> deleteUser(@PathVariable Long id,
                                          @RequestAttribute String userRole) {
        if (!"ADMIN".equals(userRole)) {
            return ApiResponse.error("权限不足，仅管理员可删除用户");
        }
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) {
        try {
            ...
            log.error("登录过程发生错误: {}", e.getMessage());
    ...
    // 获取当前用户信息
    @GetMapping("/me")
    public ApiResponse<User> getCurrentUser(@RequestAttribute Long userId) {
        logger.info("获取当前用户信息: {}", userId);
        Optional<User> user = userService.getUserById(userId);
        return user.map(value -> ApiResponse.success("获取用户信息成功", value))
                .orElse(ApiResponse.error("用户不存在"));
    }

    // 获取所有管理员用户（仅管理员）
    @GetMapping("/admins")
    public ApiResponse<List<User>> getAdminUsers(@RequestAttribute String userRole) {
        if (!"ADMIN".equals(userRole)) {
            return ApiResponse.error("权限不足，仅管理员可查看管理员列表");
        }

        logger.info("获取所有管理员用户");
        List<User> admins = userService.getAdminUsers();
        return ApiResponse.success("获取管理员列表成功", admins);
    }

    // 获取所有学生用户（仅管理员）
    @GetMapping("/students")
    public ApiResponse<List<User>> getStudentUsers(@RequestAttribute String userRole) {
        if (!"ADMIN".equals(userRole)) {
            return ApiResponse.error("权限不足，仅管理员可查看学生列表");
        }
    ...
    public ApiResponse<Map<String, Object>> checkPermission(@RequestAttribute Long userId,
                                                            @RequestAttribute String userRole) {
        ...
        return ApiResponse.success("权限检查成功", permissionInfo);
    }
    ...
    public ApiResponse<User> register(@RequestBody User user) {
        ...
        try {
            ...
            return ApiResponse.success("用户注册成功", createdUser);
        } catch (RuntimeException e) {
